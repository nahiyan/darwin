...
namespace OpenNN
{
    ...
    BoundingLayer::BoundingLayer(const onntinyxml2::XMLDocument &bounding_layer_document) : Layer()
    {
    ...
    void BoundingLayer::set(const onntinyxml2::XMLDocument &bounding_layer_document)
    {
        set_default();
    ...
    void BoundingLayer::set_bounds(const Vector<Vector<double>> &new_bounds)
    {
        ...
        else if (new_bounds[0].size() != neurons_number && new_bounds[1].size() != neurons_number)
        {
            ostringstream buffer;
    ...
    onntinyxml2::XMLDocument *BoundingLayer::to_XML() const
    {
        onntinyxml2::XMLDocument *document = new onntinyxml2::XMLDocument;

        ostringstream buffer;

        onntinyxml2::XMLElement *bounding_layer_element = document->NewElement("BoundingLayer");

        document->InsertFirstChild(bounding_layer_element);

        // Scaling neurons number

        onntinyxml2::XMLElement *size_element = document->NewElement("BoundingNeuronsNumber");
        ...
        for (size_t i = 0; i < neurons_number; i++)
        {
            onntinyxml2::XMLElement *item_element = document->NewElement("Item");
            ...
            onntinyxml2::XMLElement *lower_bound_element = document->NewElement("LowerBound");
            item_element->LinkEndChild(lower_bound_element);

            buffer.str("");
            buffer << lower_bounds[i];

            onntinyxml2::XMLText *lower_bound_text = document->NewText(buffer.str().c_str());
            lower_bound_element->LinkEndChild(lower_bound_text);

            // Upper bound

            onntinyxml2::XMLElement *upper_bound_element = document->NewElement("UpperBound");
            item_element->LinkEndChild(upper_bound_element);

            buffer.str("");
            buffer << upper_bounds[i];

            onntinyxml2::XMLText *upper_bound_text = document->NewText(buffer.str().c_str());
            upper_bound_element->LinkEndChild(upper_bound_text);
        }

        // Bounding method

        onntinyxml2::XMLElement *method_element = document->NewElement("UseBoundingLayer");
        ...
        onntinyxml2::XMLText *method_text = document->NewText(buffer.str().c_str());
    ...
    void BoundingLayer::write_XML(onntinyxml2::XMLPrinter &file_stream) const
    {
        ostringstream buffer;
    ...
    void BoundingLayer::from_XML(const onntinyxml2::XMLDocument &document)
    {
        ostringstream buffer;

        const onntinyxml2::XMLElement *bounding_layer_element = document.FirstChildElement("BoundingLayer");
        ...
        const onntinyxml2::XMLElement *neurons_number_element = bounding_layer_element->FirstChildElement("BoundingNeuronsNumber");
        ...
        {
            const onntinyxml2::XMLElement *item_element = start_element->NextSiblingElement("Item");
            start_element = item_element;

            if (!item_element)
            {
                buffer << "OpenNN Exception: BoundingLayer class.\n"
                       << "void from_XML(const onntinyxml2::XMLElement*) method.\n"
                       << "Item " << i + 1 << " is nullptr.\n";
            ...
            const onntinyxml2::XMLElement *lower_bound_element = item_element->FirstChildElement("LowerBound");
            ...
            const onntinyxml2::XMLElement *upper_bound_element = item_element->FirstChildElement("UpperBound");
        ...
        // Use bounding layer
        {
            const onntinyxml2::XMLElement *use_bounding_layer_element = bounding_layer_element->FirstChildElement("UseBoundingLayer");
        ...
        //  // Lower bounds
        //  {
        //     const onntinyxml2::XMLElement* lower_bounds_element = document.FirstChildElement("LowerBounds");
        ...
        //  // Upper bounds
        //  {
        //     const onntinyxml2::XMLElement* upper_bounds_element = document.FirstChildElement("UpperBounds");
        ...
        //  // Display
        //  {
        //     const onntinyxml2::XMLElement* display_element = document.FirstChildElement("Display");
