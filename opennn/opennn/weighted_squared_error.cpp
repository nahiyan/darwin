...
namespace OpenNN
{
    ...
    WeightedSquaredError::WeightedSquaredError(const onntinyxml2::XMLDocument &weighted_squared_error_document)
...
#pragma omp critical
    ...
    LossIndex::FirstOrderLoss WeightedSquaredError::calculate_batch_first_order_loss(const Vector<size_t> &batch_indices) const
    {
        ...
        const Tensor<double> output_gradient = calculate_output_gradient(forward_propagation[layers_number - 1].activations, targets) / training_normalization_coefficient;
...
#pragma omp critical
    ...
    onntinyxml2::XMLDocument *WeightedSquaredError::to_XML() const
    {
        ...
        onntinyxml2::XMLElement *weighted_squared_error_element = document->NewElement("WeightedSquaredError");

        document->InsertFirstChild(weighted_squared_error_element);

        // Positives weight
        {
            onntinyxml2::XMLElement *element = document->NewElement("PositivesWeight");
            weighted_squared_error_element->LinkEndChild(element);

            buffer.str("");
            buffer << positives_weight;

            onntinyxml2::XMLText *text = document->NewText(buffer.str().c_str());
            element->LinkEndChild(text);
        }

        // Negatives weight
        {
            onntinyxml2::XMLElement *element = document->NewElement("NegativesWeight");
            weighted_squared_error_element->LinkEndChild(element);
        ...
        // Display
        //   {
        //      onntinyxml2::XMLElement* element = document->NewElement("Display");
        //      weighted_squared_error_element->LinkEndChild(element);
    ...
    void WeightedSquaredError::write_XML(onntinyxml2::XMLPrinter &file_stream) const
    {
        ostringstream buffer;
    ...
    void WeightedSquaredError::from_XML(const onntinyxml2::XMLDocument &document)
    {
        const onntinyxml2::XMLElement *root_element = document.FirstChildElement("WeightedSquaredError");
        ...
        const onntinyxml2::XMLElement *error_element = root_element->FirstChildElement("Error");

        const onntinyxml2::XMLElement *positives_weight_element = error_element->FirstChildElement("PositivesWeight");
        ...
        const onntinyxml2::XMLElement *negatives_weight_element = error_element->FirstChildElement("NegativesWeight");
        ...
        onntinyxml2::XMLDocument regularization_document;
        onntinyxml2::XMLNode *element_clone;

        const onntinyxml2::XMLElement *regularization_element = root_element->FirstChildElement("Regularization");
