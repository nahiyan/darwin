...
void pool::mutate_link(genome& g, bool force_bias)
{
    ...
    auto is_output = [&](unsigned int node) -> bool { return node < this->network_info.functional_nodes && node >= (this->network_info.input_size + this->network_info.bias_size); };
...
void pool::new_generation()
{
    ...
    unsigned int sum = this->total_average_fitness();
    for (auto s = this->species.begin(); s != this->species.end(); s++) {
        unsigned int breed = std::floor(((1. * (*s).average_fitness) / (1. * sum)) * 1. * this->speciating_parameters.population) - 1;
