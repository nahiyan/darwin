...
public:
    ...
    void import_from_stream(std::stringstream& ss)
    {
        this->nodes.clear();
        this->input_nodes.clear();
        this->output_nodes.clear();
    ...
    void export_to_stream(std::stringstream& ss)
    {
        if (this->recurrent)
            ss << "recurrent" << std::endl;
        else
            ss << "non-recurrent" << std::endl;
        ss << nodes.size() << std::endl
           << std::endl;

        for (size_t i = 0; i < nodes.size(); i++) {
            ss << nodes[i].type << " ";
            ss << nodes[i].in_nodes.size() << std::endl;
            for (unsigned int j = 0; j < nodes[i].in_nodes.size(); j++)
                ss << nodes[i].in_nodes[j].first << " "
                   << nodes[i].in_nodes[j].second << " ";
            ss << std::endl
               << std::endl;
        }
