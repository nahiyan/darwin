...
namespace OpenNN
{
    ...
    TestingAnalysis::TestingAnalysis(const onntinyxml2::XMLDocument &testing_analysis_document)
        : neural_network_pointer(nullptr),
          data_set_pointer(nullptr)
    {
        set_default();
...
#pragma omp parallel for reduction(+ \
    ...
    double TestingAnalysis::calculate_area_under_curve_confidence_limit(const Tensor<double> &targets, const Tensor<double> &outputs) const
    {
        ...
        const double confidence_limit = 1.64485 * sqrt((area_under_curve * (1.0 - area_under_curve) + (total_positives - 1.0) * (Q_1 - area_under_curve * area_under_curve) + (total_negatives - 1.0) * (Q_2 - area_under_curve * area_under_curve)) / (total_positives * total_negatives));
...
#pragma omp parallel for
    ...
    Vector<double> TestingAnalysis::calculate_maximum_gain(const Matrix<double> &positive_cumulative_gain, const Matrix<double> &negative_cumulative_gain) const
    {
        ...
        for (size_t i = 0; i < points_number - 1; i++)
        {
            ...
            if (positive_cumulative_gain(i + 1, 1) - negative_cumulative_gain(i + 1, 1) > maximum_gain[1] && positive_cumulative_gain(i + 1, 1) - negative_cumulative_gain(i + 1, 1) > 0.0)
            {
    ...
    onntinyxml2::XMLDocument *TestingAnalysis::to_XML() const
    {
        ...
        onntinyxml2::XMLElement *testing_analysis_element = document->NewElement("TestingAnalysis");
    ...
    void TestingAnalysis::write_XML(onntinyxml2::XMLPrinter &file_stream) const
    {
        ostringstream buffer;
    ...
    void TestingAnalysis::from_XML(const onntinyxml2::XMLDocument &document)
    {
        ostringstream buffer;

        const onntinyxml2::XMLElement *root_element = document.FirstChildElement("TestingAnalysis");
