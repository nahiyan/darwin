...
namespace OpenNN
{
    ...
    TrainingStrategy::TrainingStrategy(const onntinyxml2::XMLDocument &document)
    {
        set_optimization_method(QUASI_NEWTON_METHOD);
    ...
    {
        if (loss_method == SUM_SQUARED_ERROR && sum_squared_error_pointer != nullptr)
            return sum_squared_error_pointer;
        else if (loss_method == MEAN_SQUARED_ERROR && mean_squared_error_pointer != nullptr)
            return mean_squared_error_pointer;
        else if (loss_method == NORMALIZED_SQUARED_ERROR && normalized_squared_error_pointer != nullptr)
            return normalized_squared_error_pointer;
        else if (loss_method == MINKOWSKI_ERROR && Minkowski_error_pointer != nullptr)
            return Minkowski_error_pointer;
        else if (loss_method == WEIGHTED_SQUARED_ERROR && weighted_squared_error_pointer != nullptr)
            return weighted_squared_error_pointer;
        else if (loss_method == CROSS_ENTROPY_ERROR && cross_entropy_error_pointer != nullptr)
    ...
    {
        if (optimization_method == GRADIENT_DESCENT && gradient_descent_pointer != nullptr)
            return gradient_descent_pointer;
        else if (optimization_method == CONJUGATE_GRADIENT && conjugate_gradient_pointer != nullptr)
            return conjugate_gradient_pointer;
        else if (optimization_method == QUASI_NEWTON_METHOD && quasi_Newton_method_pointer != nullptr)
            return quasi_Newton_method_pointer;
        else if (optimization_method == LEVENBERG_MARQUARDT_ALGORITHM && Levenberg_Marquardt_algorithm_pointer != nullptr)
            return Levenberg_Marquardt_algorithm_pointer;
        else if (optimization_method == STOCHASTIC_GRADIENT_DESCENT && stochastic_gradient_descent_pointer != nullptr)
            return stochastic_gradient_descent_pointer;
        else if (optimization_method == ADAPTIVE_MOMENT_ESTIMATION && adaptive_moment_estimation_pointer != nullptr)
    ...
    onntinyxml2::XMLDocument *TrainingStrategy::to_XML() const
    {
        ostringstream buffer;
        ...
        onntinyxml2::XMLElement *training_strategy_element = document->NewElement("TrainingStrategy");
        ...
        case GRADIENT_DESCENT:
        {
            onntinyxml2::XMLElement *main_element = document->NewElement("Main");
            training_strategy_element->LinkEndChild(main_element);

            main_element->SetAttribute("Type", "GRADIENT_DESCENT");

            const onntinyxml2::XMLDocument *gradient_descent_document = gradient_descent_pointer->to_XML();

            const onntinyxml2::XMLElement *gradient_descent_element = gradient_descent_document->FirstChildElement("GradientDescent");

            for (const onntinyxml2::XMLNode *nodeFor = gradient_descent_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
        ...
        case CONJUGATE_GRADIENT:
        {
            ...
            const onntinyxml2::XMLDocument *conjugate_gradient_document = conjugate_gradient_pointer->to_XML();

            const onntinyxml2::XMLElement *conjugate_gradient_element = conjugate_gradient_document->FirstChildElement("ConjugateGradient");

            for (const onntinyxml2::XMLNode *nodeFor = conjugate_gradient_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
                main_element->InsertEndChild(copy);
            }
        ...
        case QUASI_NEWTON_METHOD:
        {
            ...
            const onntinyxml2::XMLDocument *quasi_Newton_method_document = quasi_Newton_method_pointer->to_XML();

            const onntinyxml2::XMLElement *quasi_Newton_method_element = quasi_Newton_method_document->FirstChildElement("QuasiNewtonMethod");

            for (const onntinyxml2::XMLNode *nodeFor = quasi_Newton_method_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
                main_element->InsertEndChild(copy);
            }
        ...
        case LEVENBERG_MARQUARDT_ALGORITHM:
        {
            ...
            const onntinyxml2::XMLDocument *Levenberg_Marquardt_algorithm_document = Levenberg_Marquardt_algorithm_pointer->to_XML();

            const onntinyxml2::XMLElement *Levenberg_Marquardt_algorithm_element = Levenberg_Marquardt_algorithm_document->FirstChildElement("LevenbergMarquardtAlgorithm");

            for (const onntinyxml2::XMLNode *nodeFor = Levenberg_Marquardt_algorithm_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
                main_element->InsertEndChild(copy);
            }
        ...
        case STOCHASTIC_GRADIENT_DESCENT:
        {
            ...
            const onntinyxml2::XMLDocument *stochastic_gradient_descent_document = stochastic_gradient_descent_pointer->to_XML();

            const onntinyxml2::XMLElement *stochastic_gradient_descent_element = stochastic_gradient_descent_document->FirstChildElement("StochasticGradientDescent");

            for (const onntinyxml2::XMLNode *nodeFor = stochastic_gradient_descent_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
                main_element->InsertEndChild(copy);
            }
        ...
        case ADAPTIVE_MOMENT_ESTIMATION:
        {
            ...
            const onntinyxml2::XMLDocument *adaptive_moment_estimation_document = adaptive_moment_estimation_pointer->to_XML();

            const onntinyxml2::XMLElement *adaptive_moment_estimation_element = adaptive_moment_estimation_document->FirstChildElement("AdaptiveMomentEstimation");

            for (const onntinyxml2::XMLNode *nodeFor = adaptive_moment_estimation_element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
            {
                onntinyxml2::XMLNode *copy = nodeFor->DeepClone(document);
                main_element->InsertEndChild(copy);
            }
    ...
    void TrainingStrategy::write_XML(onntinyxml2::XMLPrinter &file_stream) const
    {
    ...
    void TrainingStrategy::from_XML(const onntinyxml2::XMLDocument &document)
    {
        const onntinyxml2::XMLElement *root_element = document.FirstChildElement("TrainingStrategy");
        ...
        // Loss index
        {
            ...
            const onntinyxml2::XMLElement *element = root_element->FirstChildElement("LossIndex");

            if (element)
            {
                const onntinyxml2::XMLElement *error_element = element->FirstChildElement("Error");
                ...
                case SUM_SQUARED_ERROR:
                {
                    onntinyxml2::XMLDocument new_document;

                    onntinyxml2::XMLElement *sum_squared_error_element = new_document.NewElement("SumSquaredError");

                    for (const onntinyxml2::XMLNode *nodeFor = element->FirstChild(); nodeFor; nodeFor = nodeFor->NextSibling())
                    {
                        onntinyxml2::XMLNode *copy = nodeFor->DeepClone(&new_document);
                ...
                case MEAN_SQUARED_ERROR:
                {
                    ...
                    onntinyxml2::XMLElement *mean_squared_error_element = new_document.NewElement("MeanSquaredError");
                ...
                case NORMALIZED_SQUARED_ERROR:
                {
                    ...
                    onntinyxml2::XMLElement *normalized_squared_error_element = new_document.NewElement("NormalizedSquaredError");
                ...
                case MINKOWSKI_ERROR:
                {
                    ...
                    onntinyxml2::XMLElement *Minkowski_error_element = new_document.NewElement("MinkowskiError");
                ...
                case CROSS_ENTROPY_ERROR:
                {
                    ...
                    onntinyxml2::XMLElement *cross_entropy_error_element = new_document.NewElement("CrossEntropyError");
                ...
                case WEIGHTED_SQUARED_ERROR:
                {
                    ...
                    onntinyxml2::XMLElement *weighted_squared_error_element = new_document.NewElement("WeightedSquaredError");
        ...
        // Main
        {
            const onntinyxml2::XMLElement *element = root_element->FirstChildElement("Main");
            ...
            if (element)
            {
                ...
                case GRADIENT_DESCENT:
                {
                    ...
                    onntinyxml2::XMLElement *gradient_descent_element = new_document.NewElement("GradientDescent");
                ...
                case CONJUGATE_GRADIENT:
                {
                    ...
                    onntinyxml2::XMLElement *conjugate_gradient_element = new_document.NewElement("ConjugateGradient");
                ...
                case QUASI_NEWTON_METHOD:
                {
                    ...
                    onntinyxml2::XMLElement *quasi_newton_method_element = new_document.NewElement("QuasiNewtonMethod");
                ...
                case LEVENBERG_MARQUARDT_ALGORITHM:
                {
                    ...
                    onntinyxml2::XMLElement *levenberg_marquardt_algorithm_element = new_document.NewElement("LevenbergMarquardtAlgorithm");
                ...
                case STOCHASTIC_GRADIENT_DESCENT:
                {
                    ...
                    onntinyxml2::XMLElement *stochastic_gradient_descent_element = new_document.NewElement("StochasticGradientDescent");
                ...
                case ADAPTIVE_MOMENT_ESTIMATION:
                {
                    ...
                    onntinyxml2::XMLElement *adaptive_moment_estimation_element = new_document.NewElement("AdaptiveMomentEstimation");
    ...
    {
        onntinyxml2::XMLDocument *document = to_XML();
