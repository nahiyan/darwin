...
function prepareConfig (extension, config, initialModels) {
  ...
  preparedConfig.initial_models = initialModels
...
window.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    app.ports.startExtension.subscribe(function (info) {
      ...
      const initialModels = info[1]

      const preparedConfig = JSON.stringify(prepareConfig(extensionName, config, initialModels))

      console.log(preparedConfig)

      // exec(`../../extensions/bin/${extensionName} "${preparedConfig}"`, function (error, stdout, stderr) {
      //   if (error) {
