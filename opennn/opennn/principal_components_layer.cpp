...
/// @param outputs_names Name of outputs from the principal components.*/
    ...
    // onntinyxml2::XMLDocument* to_XML() const method

    /// Serializes the principal components layer object into a XML document of the TinyXML library.
    /// See the OpenNN manual for more information about the format of this element.

    onntinyxml2::XMLDocument *PrincipalComponentsLayer::to_XML() const
    {
        onntinyxml2::XMLDocument *document = new onntinyxml2::XMLDocument;
        /*
    ostringstream buffer;

    onntinyxml2::XMLElement* principal_components_layer_element = document->NewElement("PrincipalComponentsLayer");

    document->InsertFirstChild(principal_components_layer_element);

    // Principal components neurons number

    onntinyxml2::XMLElement* size_element = document->NewElement("PrincipalComponentsNeuronsNumber");
    ...
    {
        onntinyxml2::XMLElement* principal_components_element = document->NewElement("PrincipalComponents");
        ...
        onntinyxml2::XMLElement* eigenvector_element = document->NewElement("Eigenvector");
        principal_components_element->LinkEndChild(eigenvector_element);

        buffer.str("");
        buffer << principal_components.get_row(i);

        onntinyxml2::XMLText* eigenvector_text = document->NewText(buffer.str().c_str());
        eigenvector_element->LinkEndChild(eigenvector_text);
    }

    // Means

    onntinyxml2::XMLElement* means_element = document->NewElement("Means");
    means_element->LinkEndChild(means_element);

    buffer.str("");
    buffer << means;

    onntinyxml2::XMLText* means_text = document->NewText(buffer.str().c_str());
    means_element->LinkEndChild(means_text);

    // Principal components method

    onntinyxml2::XMLElement* method_element = document->NewElement("PrincipalComponentsMethod");
    principal_components_layer_element->LinkEndChild(method_element);

    onntinyxml2::XMLText* method_text = document->NewText(write_principal_components_method().c_str());
...
*/
    ...
    void PrincipalComponentsLayer::write_XML(onntinyxml2::XMLPrinter &file_stream) const
    {
        ostringstream buffer;
    ...
    void PrincipalComponentsLayer::from_XML(const onntinyxml2::XMLDocument &document)
    {
        ostringstream buffer;

        const onntinyxml2::XMLElement *principal_components_layer_element = document.FirstChildElement("PrincipalComponentsLayer");
        ...
        const onntinyxml2::XMLElement *inputs_number_element = principal_components_layer_element->FirstChildElement("InputsNumber");
        ...
        const onntinyxml2::XMLElement *principal_components_number_element = principal_components_layer_element->FirstChildElement("PrincipalComponentsNumber");
        ...
        if (principal_components_number != 0)
        {
            ...
            const onntinyxml2::XMLElement *means_element = principal_components_layer_element->FirstChildElement("Means");
            ...
            const onntinyxml2::XMLElement *explained_variance_element = principal_components_layer_element->FirstChildElement("ExplainedVariance");
            ...
            const onntinyxml2::XMLElement *start_element = means_element;

            for (size_t i = 0; i < inputs_number /*principal_components_number*/; i++)
            {
                const onntinyxml2::XMLElement *principal_components_element = start_element->NextSiblingElement("PrincipalComponent");
        ...
        const onntinyxml2::XMLElement *principal_components_method_element = principal_components_layer_element->FirstChildElement("PrincipalComponentsMethod");
