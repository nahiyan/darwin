...
namespace OpenNN
{
   ...
   LearningRateAlgorithm::LearningRateAlgorithm(const onntinyxml2::XMLDocument &document)
       : loss_index_pointer(nullptr)
   {
      from_XML(document);
   }
   ...
   {
      const double c = -(triplet.A.second * (triplet.U.first - triplet.B.first) + triplet.U.second * (triplet.B.first - triplet.A.first) + triplet.B.second * (triplet.A.first - triplet.U.first)) / ((triplet.A.first - triplet.U.first) * (triplet.U.first - triplet.B.first) * (triplet.B.first - triplet.A.first));
      ...
      const double b = (triplet.A.second * (triplet.U.first * triplet.U.first - triplet.B.first * triplet.B.first) + triplet.U.second * (triplet.B.first * triplet.B.first - triplet.A.first * triplet.A.first) + triplet.B.second * (triplet.A.first * triplet.A.first - triplet.U.first * triplet.U.first)) / ((triplet.A.first - triplet.U.first) * (triplet.U.first - triplet.B.first) * (triplet.B.first - triplet.A.first));
   ...
   onntinyxml2::XMLDocument *LearningRateAlgorithm::to_XML() const
   {
      ostringstream buffer;
      ...
      onntinyxml2::XMLElement *root_element = document->NewElement("LearningRateAlgorithm");
   ...
   void LearningRateAlgorithm::write_XML(onntinyxml2::XMLPrinter &file_stream) const
   {
      ostringstream buffer;
   ...
   void LearningRateAlgorithm::from_XML(const onntinyxml2::XMLDocument &document)
   {
      const onntinyxml2::XMLElement *root_element = document.FirstChildElement("LearningRateAlgorithm");
      ...
      // Training rate method
      {
         const onntinyxml2::XMLElement *element = root_element->FirstChildElement("LearningRateMethod");
      ...
      // Loss tolerance
      {
         const onntinyxml2::XMLElement *element = root_element->FirstChildElement("LossTolerance");
